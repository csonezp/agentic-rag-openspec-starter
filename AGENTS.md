...
- 每个学习小节的学习文档必须包含以下稳定栏目：
  - `本次做了什么`：说明本小节实际新增、修改或验证了哪些内容。
  - `原理/流程`：解释本小节背后的核心原理，以及数据、控制流或调用链路如何运转。
  - `验证记录`：记录执行过的测试、真实调用、dry-run、失败修复和观察结果。
  - `本次交互总结`：记录本次学习中用户追问、概念澄清、临时规则和关键决策。
