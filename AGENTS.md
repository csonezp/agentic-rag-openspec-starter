...
- 开始新的学习小节前，如果上一个已完成小节的学习分支尚未合并回 `main`，必须先完成提交、合并回 `main`，再从最新 `main` 创建新的学习分支。
- 不要主动删除旧学习分支；无论本地还是远程旧分支，默认保留，只有在用户明确要求删除时才可以删除。
...
- 学习过程中与用户产生的关键交互内容也要总结进对应学习文档，包括用户追问、概念澄清、实现取舍、验证方式和临时新增的协作规则；不要只记录代码改动。
